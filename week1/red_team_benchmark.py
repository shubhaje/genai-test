...
def readjsonFile(filename):
    ...
    with open(filename, "r") as f:
        data = json.load(f)
   # print(f"📂 Successfully read data from {filename}", data)
    for item in data["attacks"]:
        #print(item)
        print(f"- {item['attack_category']}: {item['example_prompt']}")
        print("-----------------------------")
       # return True
...
if __name__ == "__main__":
   ...
   # print(f"📂 Successfully read data from {filename}", data)
    for item in data["attacks"]:
        #print(item)
       # adversarial_case=f"{item['attack_category']}: {item['example_prompt']}"
        adversarial_case=LLMTestCase(
        input=f"{item['attack_category']}",
        actual_output=f"{item['example_prompt']}"
)
        print("-----------------------------")
        print(adversarial_case)
        print("-----------------------------")
       # print(f"- {item['attack_category']}: {item['example_prompt']}")
      #  print("-----------------------------")
   #Run tests
